/**
   ...
   @code
   g++ -std=c++11 -o MSSM_H MSSM_H.cpp -I/$GAMBIT/DecayBit/include/gambit/DecayBit/
   @endcode
...
int main() {
  ...
  for (int i = 0; i <= 3; i += 1) {
    for (int j = 0; j <= 3; j += 1) {
      std::cout << Gambit::DecayBit::MSSM_H::gamma_h_chi_0(i, j, m_0, Z, -8.46891780E-01)
                << " " << i + 1
                << " " << j + 1 <<  std::endl;
    }
  }
  ...
  for (int i = 0; i <= 1; i += 1) {
    for (int j = 0; j <= 1; j += 1) {
      std::cout << Gambit::DecayBit::MSSM_H::gamma_h_chi_pm(i, j, m_pm, U, V, -8.46891780E-01)
                << " " << i + 1
                << " " << j + 1 <<  std::endl;
    }
  }
